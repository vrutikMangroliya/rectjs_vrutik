...
const ProductData = [
  ...
  },
  {
    ...
    description:
      "original fauji cereal muesli 250gm box pack original fauji cereals muesli fruit nuts.",
    price: 46,
  ...
  },
  {
    ...
    description:
      "Package Include 6 Birds with Adhesive Tape Shape: 3D Shaped Wooden Birds Material.",
    price: 51,
    discountPercentage: 15.58,
    rating: 4.41,
    stock: 17,
...
const Searchbar = () => {
  ...
  const ProductList = ProductData.filter((val) => {
    if (searchTerm === "") {
      return val;
    } else if (
  ...
  return (
    <>
      <div>
        <h1 className="text-center text-white p-6 bg-purple-700 text-3xl">
          React Filter Rendering Lists
        </h1>
        <div className="flex justify-center mt-10">
          <div className="relative">
            <IoIosSearch className="absolute top-2.5 left-2 size-5 text-blue-500 text-2xl" />
          </div>
          <input
            type="text"
            className="px-6 py-2 w-full bg-slate-300 rounded-md focus:outline-none"
        ...
        <div className="flex justify-center items-center gap-2 flex-wrap mt-20 px-4">
          {ProductList.map((val) => {
            return (
              <>
                <div className="size-[430px] border-2 border-solid pl-2 mb-4 ">
                  <div className="flex justify-center items-center">
                    <img
                      className="size-72 hover:translate-y-3 transition-all ease-linear duration-300"
                      src={val.images}
                    />
                  </div>
                  <div className="text-center">
                    <p className="my-2 font-mono text-2xl text-fuchsia-500 capitalize">
                      {val.title}
                    </p>
                    <p className="my-2 text-rose-500 ">{val.description}</p>
